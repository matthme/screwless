...
export class OfferDetail extends LitElement {
   ...
   * @internal
   */
   _offer = new StoreSubscriber(this, () => this.offersStore.offers.get(this.offerHash));
  ...
  async deleteOffer() {
    try {
      ...
      this.dispatchEvent(new CustomEvent('offer-deleted', {
        bubbles: true,
        composed: true,
        detail: {
  ...
  renderDetail(entryRecord: EntryRecord<Offer>) {
    return html`<sl-card>
          ...
          <sl-icon-button style="margin-left: 8px" .src=${wrapPathInSvg(mdiDelete)} @click=${() => this.deleteOffer()}></sl-icon-button>
        </div>

        <div style="display: flex; flex-direction: column">
  
          <div style="display: flex; flex-direction: column; margin-bottom: 16px">
	    <span style="margin-bottom: 8px"><strong>${msg("Amount")}</strong></span>
 	    <span style="white-space: pre-line">${ entryRecord.entry.amount }</span>
	  </div>

          <div style="display: flex; flex-direction: column; margin-bottom: 16px">
	    <span style="margin-bottom: 8px"><strong>${msg("Offered Currency")}</strong></span>
 	    <span style="white-space: pre-line">${ entryRecord.entry.offered_currency }</span>
	  </div>

          <div style="display: flex; flex-direction: column; margin-bottom: 16px">
	    <span style="margin-bottom: 8px"><strong>${msg("Requested Currency")}</strong></span>
 	    <span style="white-space: pre-line">${ entryRecord.entry.requested_currency }</span>
	  </div>

          <div style="display: flex; flex-direction: column; margin-bottom: 16px">
	    <span style="margin-bottom: 8px"><strong>${msg("Available From")}</strong></span>
 	    <span style="white-space: pre-line"><sl-format-date .date=${new Date(entryRecord.entry.available_from / 1000) }></sl-format-date></span>
	  </div>

          <div style="display: flex; flex-direction: column; margin-bottom: 16px">
	    <span style="margin-bottom: 8px"><strong>${msg("Available Until")}</strong></span>
 	    <span style="white-space: pre-line"><sl-format-date .date=${new Date(entryRecord.entry.available_until / 1000) }></sl-format-date></span>
	  </div>

          <div style="display: flex; flex-direction: column; margin-bottom: 16px">
	    <span style="margin-bottom: 8px"><strong>${msg("Airport")}</strong></span>
 	    <span style="white-space: pre-line">${ entryRecord.entry.airport }</span>
	  </div>
  ...
  render() {
    switch (this._offer.value.status) {
      ...
      case "complete":
        ...
        if (!offer) return html`<span>${msg("The requested offer doesn't exist")}</span>`;
    
        if (this._editing) {
    	  return html`<edit-offer
    	    .originalOfferHash=${this.offerHash}
    	    .currentRecord=${ offer }
            @offer-updated=${async () => { this._editing = false; } }
